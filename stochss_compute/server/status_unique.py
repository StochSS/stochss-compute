'''
stochss_compute.server.status_unique
'''
...
class StatusUniqueHandler(RequestHandler):
    '''
    Endpoint for requesting the status of a unique (one-off) simulation.
    '''
    ...
    async def get(self, unique_key):
        '''
        Process GET request.

        :param unique_key: ID of the running simulation. Required.
        :type unique_key: str
        '''
        if unique_key == '':
