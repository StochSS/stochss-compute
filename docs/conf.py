...
stochss_compute = {}
with open(path.join(ROOT_DIR, 'stochss_compute/__version__.py'), encoding='utf-8') as f:
    text = f.read().rstrip().splitlines()
    variables = [line for line in text if line.startswith('__') and '=' in line]
    for v in variables:
        setting = v.split('=')
        stochss_compute[setting[0].strip()] = setting[1].strip().replace("'", '')


project   = stochss_compute['__title__']
copyright = stochss_compute['__copyright__']
author    = stochss_compute['__author__']

# The short X.Y version
version   = stochss_compute['__version__']
version   = version[:version.rfind('.')]

# The full version, including alpha/beta/rc tags
release   = stochss_compute['__version__']
...
htmlhelp_basename = 'StochSS-Compute-doc'
...
latex_documents = [
    (master_doc, 'StochSS-Compute.tex', 'StochSS-Compute Documentation',
     'Author', 'manual'),
]
...
man_pages = [
    (master_doc, 'StochSS-Compute', 'StochSS-Compute Documentation',
     [author], 1)
]
...
html_theme_options = {
    ...
    'fixed_sidebar': 'true',
    'github_banner': 'true',
    'logo': 'img/stochss-compute-logo.svg',
    'touch_icon': 'img/stochss-compute-logo.svg',
